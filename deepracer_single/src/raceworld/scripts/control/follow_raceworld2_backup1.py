...
class follow_switch:
    ...
    def follow_line(self, image):
        ...
        if cnts.shape[0] > 10:
            ...
            if self.tag == 0:
                self.speed = 2.8-(np.log(2*abs(self.ang)+1))*3.0
                self.ang = Sigmoid_01(
                    self.err_sum**3+1.3*self.err_sum, 0.5, 4.3) + 0.3*(self.err_sum-self.last_err_sum)
                akm.drive.speed = self.speed
                akm.drive.steering_angle = self.ang
            if self.tag == 2:
            ...
            if self.tag == 1:
                ...
                self.ang = Sigmoid_01(
                    self.err_sum**3+1.3*self.err_sum, 0.5, 5) + 0.45*(self.err_sum-self.last_err_sum)  # 0.366565
                # self.ang = Sigmoid_01(
