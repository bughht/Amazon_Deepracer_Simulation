    ...
    def follow_line(self, image):
        ...
        if cnts.shape[0] > 10:
            ...
            if self.tag == -1:
                ...
                self.speed = 3.3-(np.log(2*abs(self.ang)+1))*3.25
            ...
            if self.tag == 0:
                ...
                self.ang = Sigmoid_01(
                    self.err_sum**3+1.3*self.err_sum, 0.5, 4.5) + 0.3*(self.err_sum-self.last_err_sum)
                akm.drive.speed = self.speed
                akm.drive.steering_angle = self.ang
            if self.tag == 2:
                self.speed = 2.5-(np.log(2*abs(self.ang)+1))*3
                # print(self.speed)
                # self.speed = min(self.speed, 2.5)
                # self.ang *= 1.02
                self.ang = Sigmoid_01(
                    self.err_sum**3+1.3*self.err_sum, 0.5, 6) + 0.4551*(self.err_sum-self.last_err_sum)
                akm.drive.speed = self.speed
                akm.drive.steering_angle = self.ang
            if self.tag == 1:
                # akm.drive.acceleration = 1
                # akm.drive.steering_angle_velocity = -1.5
                self.speed = 3-(np.log(2*abs(self.ang)+1))*3.5
                self.ang = Sigmoid_01(
