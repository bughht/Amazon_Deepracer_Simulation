...
class Key_Cam:
    def __init__(self) -> None:
        ...
        self.pub = rospy.Publisher(
            "/deepracer1/ackermann_cmd_mux/output", AckermannDriveStamped, queue_size=10)
        self.pub_key()

    def openvino_init(self):
        self.ie = IECore()
        self.net = self.ie.read_network(model="/home/bughht/deepracer/Electro_RaceCar/models/ssdlite_FP32/ssdlite_mobilenet_v2.xml",
                                        weights='/home/bughht/deepracer/Electro_RaceCar/models/ssdlite_FP32/ssdlite_mobilenet_v2.bin')
    ...
    def detect_QR_STOP(self):
        ...
        for frame_idx in range(2):
            ...
            if COCO_STOPSIGN in self.result_ir[frame_idx, :, 1]:
                SIGN_idx = np.where(
                    self.result_ir[frame_idx, :, 1] == COCO_STOPSIGN)
    ...
    def combine_plot(self):
        ...
        for frame_idx in range(2):
            if self.QR_TAGS[frame_idx]:
                for tag in self.QR_TAGS[frame_idx]:
                    self.frame_aug[frame_idx] = cv.drawContours(self.frame_aug[frame_idx], [
                                                                np.expand_dims(tag.corners.astype('int32'), 0)], -1, (0, 0, 255), 3)
            if self.STOP_cord[frame_idx][0] != 0:
                top_left = tuple(
                    (self.STOP_cord[frame_idx, 0:2]*CAM_SIZE[:2][::-1]).astype(int))
                bottom_right = tuple(
                    (self.STOP_cord[frame_idx, 2:4]*CAM_SIZE[:2][::-1]).astype(int))
                print(top_left, bottom_right)
                print(self.STOP_cord[frame_idx, 0:2] -
                      self.STOP_cord[frame_idx, 2:4])
                cv.rectangle(
                    self.frame_aug[frame_idx], top_left, bottom_right, (0, 255, 255), 3)

        # self.frame_STOP=``
        self.frame_c = np.concatenate(
            (self.frame_aug[0], self.frame_aug[1]), axis=1)
    ...
    def pub_key(self):
        ...
        while True:
            ...
            print("Keyboard Input:{},Speed:{},Angle:{}".format(
                key, self.speed, self.angle))
            akm.drive.speed = self.speed
            akm.drive.steering_angle = self.angle

            self.pub.publish(akm)
