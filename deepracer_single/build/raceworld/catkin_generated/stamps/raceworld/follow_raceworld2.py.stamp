    ...
    def follow_line(self, image):
        ...
        if cnts.shape[0] > 10:
            ...
            if self.tag == -1:
                ...
                err_loc_tgt = bottom_pt[0] * \
                    np.cos(np.mean([err_ang_tgt, err_rem_tgt])) - 63
            elif self.tag == 2:
                err_loc_tgt = bottom_pt[0] * \
                ...
                err_loc_tgt = bottom_pt[0] * \
                    np.cos(np.mean([err_ang_tgt, err_rem_tgt])) + 105
            ...
            if self.tag == -1:
                self.ang = Sigmoid_01(
                    self.err_sum**3+1.3*self.err_sum, 0.5, 5) + 0.4154*(self.err_sum-self.last_err_sum)
                self.speed = 3.5-(np.log(2*abs(self.ang)+1))*2.275
                # self.speed = min(self.speed, 3.0)
                akm.drive.acceleration = 0.148
                akm.drive.steering_angle_velocity = 2.2
                # akm.drive.acceleration = 0.4
                # akm.drive.steering_angle_velocity = 0.8
            if self.tag == 0:
                self.speed = 3-(np.log(2*abs(self.ang)+1))*2.8
                self.ang = Sigmoid_01(
                    self.err_sum**3+1.3*self.err_sum, 0.5, 5) + 0.3*(self.err_sum-self.last_err_sum)
            ...
            if self.tag == 1:
                ...
                self.ang = Sigmoid_01(
                    self.err_sum**3+1.3*self.err_sum, 0.5, 5) + 0.36665*(self.err_sum-self.last_err_sum)  # 0.366565
                # self.ang = Sigmoid_01(
            ...
            if self.tag == 3:
                # self.ang = Sigmoid_01(
                #     self.err_sum**3+1.3*self.err_sum, 0.5, 8) + 0.53*(self.err_sum-self.last_err_sum)  # 0.52
                self.speed = 1
                self.ang = - 0.4
